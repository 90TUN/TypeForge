      ...
      <div className={`max-w-4xl w-full ${bgSecondary} border ${borderColor} rounded-2xl p-6 sm:p-10 shadow-2xl transition-colors`}>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          ...
          <button 
            ...
            className={`flex flex-col items-center text-center p-6 border-2 ${darkMode ? 'border-gray-700 hover:border-green-500 bg-gray-800' : 'border-gray-200 hover:border-green-500 bg-gray-50'} rounded-xl transition-all hover:scale-105 active:scale-95 group`}
          >
            <Type size={48} className={`mb-4 ${darkMode ? 'text-gray-400 group-hover:text-green-400' : 'text-gray-400 group-hover:text-green-500'}`} />
            <h3 className={`font-bold ${textPrimary} mb-2`}>Test a Font</h3>
            <p className={`text-xs ${textSecondary}`}>
              Upload an existing .otf or .ttf file to preview and type with it instantly in the browser.
            </p>
          </button>
        </div>
      </div>
